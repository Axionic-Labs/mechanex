"""Natural Language schema parser for ARA.
...
class NaturalLanguageParser(SchemaParser):
    ...
    def _extract_parameters(self, description: str) -> tuple[List[ParameterSpec], List[str]]:
        ...
        param_patterns = [
            # "a required X parameter" or "required X parameter"
            r'(?:a\s+)?required\s+["\']?(\w+)["\']?\s+parameter',
            # "an optional X parameter" or "optional X parameter"
            r'(?:an?\s+)?optional\s+["\']?(\w+)["\']?\s+parameter',
            # "takes a X parameter" or "takes X parameter"
            r'takes\s+(?:a\s+)?(?:required\s+)?["\']?(\w+)["\']?\s+parameter',
            # "X parameter (type)"
            r'["\']?(\w+)["\']?\s+parameter\s*\((\w+)\)',
            # "takes a parameter called X"
        ...
        for pattern in param_patterns:
            for match in re.finditer(pattern, description, re.IGNORECASE):
                ...
                if len(groups) > 1 and groups[1]:
                    param_type = self._infer_type(groups[1])
                else:
                    param_type = self._infer_type(param_name)

                # Check if required
                is_required = self._is_required(param_name, description)
    ...
    def _is_required(self, param_name: str, description: str) -> bool:
        """Check if a parameter is required based on context."""
        # Look for explicit required/optional markers
        required_patterns = [
