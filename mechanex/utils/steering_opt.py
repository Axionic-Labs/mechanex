...
def optimize_vector_minibatch_hf(model, tokenizer, prompts, layer,
	...
	if affine_rank is not None:
		with torch.no_grad():
			...
			matrix_left = torch.einsum('rm, r -> rm', matrix_left, starting_norm/matrix_left.norm(dim=1))
			matrix_right = torch.einsum('rm, r -> rm', matrix_right, starting_norm/matrix_right.norm(dim=1))
		matrix_left.requires_grad_(True)
		matrix_right.requires_grad_(True)
	else:
		matrix_left = None
